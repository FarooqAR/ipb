...
void Unit::move(int direction)
{
	...
	{
		//vectorize thrust
		speedX += static_cast<float>(cos((angle - 90) * constants::PI / 180)*thrust);
		speedY += static_cast<float>(sin((angle - 90) * constants::PI / 180)*thrust);
		...
		if (speedXdecimal != 0)
		{
			if (count1 %static_cast<int>(1 / speedXdecimal) == 0)
				position.x += static_cast<int>(1 * (speedXdecimal / abs(speedXdecimal)));
		}
		if (speedYdecimal != 0)
		{
			if (count1%static_cast<int>(1 / speedYdecimal) == 0)
				position.y += static_cast<int>(1 * (speedYdecimal / abs(speedYdecimal)));
		}

		count1++;

		//changes position
		position.x += static_cast<int>(speedX);
		position.y += static_cast<int>(speedY);
	}
